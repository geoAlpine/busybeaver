...
## The most useful framing — (K) as a self-generated subcriticality
The kernel can be stated as a **self-consistent (mean-field) transfer-operator** problem, which we believe
is its right home. The parity is governed by `T_n` (the parity-history carry-sum), so the "scenery" driving
each step is *generated by the orbit itself* (closed loop): the governing operator depends on its own
invariant measure, `ν = L_ν ν` (a 2-adic analogue of a McKean–Vlasov stationary equation). Haar solves it;
linearizing at Haar gives `δ = (L + F)δ` with `L` the annealed operator (verified **one-step exact**) and
`F` the **self-generation feedback**. Since `L` has a full spectral gap,
> **the complete proof is equivalent to a single inequality: `‖F‖ < 1`** (subcriticality of the closed-loop
> feedback). Empirically `‖F‖ ≈ 0.04` (robust across scenery correlation structures), and the annealed part
> of `F` is null — so `‖F‖` is purely the **correlation correction** `≈ Σ_h K(h)·C(h)` (`K` a carry kernel,
> `C` the orbit's parity autocorrelation). Proving `‖F‖<1` is (K) = Mahler, but now as an explicit
> subcriticality of a self-generated cocycle. *(This is the framing we would most value a tool for: effective
> control of a single deterministic **endogenous** cocycle — a "propagation of chaos for one computable
> trajectory" — for which standard mean-field / Gibbs–Markov / RWRS theory stops at a.e. random realizations.)*
