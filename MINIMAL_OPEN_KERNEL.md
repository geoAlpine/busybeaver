...
map** `×(3/2)` on `ℤ_2`. The recurring obstacle is the **a.e. → specified** gap:
- **Fourier / Weyl / Gowers / van der Corput:** the differencing operator fixes `(3/2)^n` at every degree
  (no degree reduction); these reach only the top `Θ(log N)` digits.
- **Transfer-operator spectral gap:** controls the orbit only at the **Haar / a.e.** level (decay of
  correlations); we have not found a way to descend it to the specified algebraic orbit.
...
- The orbit is statistically indistinguishable from i.i.d. fair coins at every finite order tested
  (mod-`2^k` discrepancy at the CLT rate; lag-mutual-information `≈0`; full block entropy). So no
  finite-order / elementary statistic separates it from a "random" sequence.
- Conditioning the moving-middle digit on the controllable data — the top digits (`{n log_2 3}`, even
  ...
  middle digit, in our measurements.
...
`log_2 3` — or a precise reason it cannot? We would value most a pointer to a near-miss, a known
obstruction we have overlooked, or a field (symbolic dynamics, p-adic harmonic analysis, random
dynamical systems, additive combinatorics) where an isomorphic "specified-orbit genericity" notion lives.
A sharp "no, because …" is as useful to us as a "yes, see X."
