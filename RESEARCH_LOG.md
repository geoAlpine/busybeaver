...
## 2026-06-26l — Rank-2 search in the renewal structure RESOLVED: the hidden non-abelianness is AMENABLE (wall = amenability)
Ground the last constructive hope (recover effective rank-2 from `T=×3` then `÷2`) — `renewal_rank2.py`,
ENGINE_SURVEY update.
- The renewal map `F` is generated by branch maps `f_D(x)=(3/2)^{D+1}x+s_D`, `s_D=(2^D−3^D)/2^{D+1}∈ℤ[1/6]`
  (both primes in the shift). They are genuinely **non-abelian** (verified `f_0∘f_1≠f_1∘f_0`: same linear `27/8`,
  shifts `−3/8` vs `−1/4`).
- **But they generate a SOLVABLE group `ℤ[1/6]⋊⟨3/2⟩`** (metabelian): linear parts `{(3/2)^k}` rank-1 abelian,
  translations `ℤ[1/6]` abelian normal. **Solvable ⇒ amenable ⇒ no spectral gap / no expansion** — Bourgain–
  Gamburd/BFLM need non-amenable (semisimple). The "second direction" (translations) is amenable, the WRONG kind.
  **No effective rank-2 recovered.**
- **Deepest characterization: THE WALL IS AMENABILITY.** The orbit's entire effective symmetry — cyclic `⟨×3/2⟩`
  or the renewal affine semigroup `ℤ[1/6]⋊⟨3/2⟩` — is amenable; every single-orbit engine needs non-amenability
