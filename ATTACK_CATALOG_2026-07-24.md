...
30. **o4 スカラーへの初等的直接攻撃** — プログラム全体で**最弱の開文**は
    `freq{ρ=1} < 0.8`（実測 0.3336、劣臨界証明書 `5≤81` は Lean 済み）。この 1 行に対する
    初等組合せ論・直接数え上げでの攻撃は、系統的には行われていない。マージンが巨大
    （1519 倍）な文は、往々にして必要より粗い道具で落ちる。**(K) 攻撃の最初の的として
    #16/#17 より安い**
