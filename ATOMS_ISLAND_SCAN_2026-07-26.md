...
> **⚠ 本文は 2026-07-26 の固定形走査の記録。末尾の追記(2026-07-27, §7〜§10)で
> **18 台**に更新され、`Atoms` は歩数でパラメータ化された。数字は追記が正。
> §1 の「9〜10 エントリを固定」という留保、および §2 の軌道関連性の方法論は依然有効。**
...
# 追記(2026-07-27)— 行動的検出で **18 台**、そしてこの路線は**枯れた**

`RESUME` §3 の項目1(5/6 near-miss)を実行した結果、項目2(勾配バンドの調査)まで一挙に決着した。

## §7 歩数は機構の一部ではなかった

`atoms_island_scan.py` は各原子を**固定の遷移形**(crawl=4歩, turn=3歩, …)として照合していた。
唯一の 5/6 near-miss `1RB0RF_0LC0RA_1LE1RD_0RC---_1LA0LE_1RA0LC`(`sA=E`)を実際に追跡すると、
turn は **5 歩**で、しかも**出力配置が標準の turn と完全に一致**していた:

```
A0→1RB, B0→0LC, C1→1RD, D0→0RC, C0→1LE      (5 歩、途中で一度左に戻る)
入力 ⟨sB, p, ⟨L, 0, 0::0::R⟩⟩  →  出力 ⟨sA, p+1, ⟨1::L, 0, 1::R⟩⟩   (標準 turn と同一)
```

つまり **`(4,1,1,2,2,3)` という歩数は D の偶然**であって、rung タイルが必要とするのは各原子の
**入出力仕様だけ**である。span はそこから6つの歩数の線形関数として決まる:

```
span = (u+m+2)·cr + (u+3)·s01 + (m+1)·s10 + mk + ta + tu
```

`RungCalc.Atoms` を6つの歩数でパラメータ化し、`RungCalc.span` を導入した(`rung_core` の相分割は
`simp only [span, Nat.mul_add, Nat.mul_succ]; omega` で閉じる — 変数係数の積があるので `omega`
単独では足りない)。

## §8 行動的再走査(`atoms_flex_scan.py`)

各原子を**その厳密な入出力配置**(状態・位置・全セル)に到達する最初の歩数で照合し、複数の
テープ文脈で歩数が一致することを要求する(= Lean の `∀`-一様性)。検証は Lean、探索は Python。
...
**5/6 が 0 になった** — すなわち**この緩和は枯れた**。あと1原子で届く機械はもう存在しない。
項目2(4/6・3/6 バンドの調査)も、これで「1原子差ではない」ことが確定したので、
別の interface を作らない限り拾えない。

歩数シグネチャは2種類だけ:

| 歩数 `(cr,mk,ta,s10,s01,tu)` | span | 台数 |
|---|---|---|
| `(4,1,1,2,2,3)` | `6(u+m)+21` | 17 |
| `(4,1,1,2,2,5)` | `6(u+m)+23` | 1 |

## §9 Lean

`lean/IslandTiles.lean` を再生成(**18 台**)。生成器は各原子の**移動列から位置正規化式を導出**する
ようにした — 5歩 turn は途中で左に戻るので `p + 1 + 1 - 1` では閉じず、`p + 1 - 1 + 1 + 1 - 1`
が必要だった(歩数だけから推測すると間に合わない実例)。

`M17` が 5歩 turn の機械で、`Atoms TM17 .E .A 4 1 1 2 2 5`、span `6u+6m+23`。
コーパス `lake build` EXIT=0(83 jobs)、公理は `[propext, Quot.sound]` のみ。

**軌道関連性(`atoms_flex_scan.py §2`、各機械の自前 span を使用)**:18 台すべて 300k 歩で停止せず、
実軌道上の IN 形配置で **発火率 100%**(`M17` は 278/278、span `6u+6m+17`)。

## §10 位置づけ

**進んだ:** 17 → 18。より本質的には、interface が**機構だけを述べる形**になり、
**「1原子差の機械はもう無い」という枯渇が証明的に確認**された。

**進んでいない:** 18 台すべて `[OPEN]`。タイルは1補題。D の RF-4(`k+1` turn 相、閉形式なし)は
依然として最大の残ギャップで、本追記は一切触れていない。
