...
1. **D の形式化** ★ 単一最良ターゲット。純 `(1 0)` COMB、milestone 族均一(状態 A、head-step −8k、
   偶 `0^33 ++ pow10 k` / 奇 `0^2 (10)^4 0^6 ++ pow10 k`)、赤旗なし、M0 groundwork 済
   (`PHASEB_D_M0_2026-07-26.md`:milestone 族、epoch 分解、anti-vacuity アンカー)。
   反転形 `D^R = 1LB0LA_1RC0RE_0RD0RB_1LA0RF_1RB0LD_1RD---` で作業(x2 同様に右成長)。
   **必須:** 最初から ∀-parametric(M2)。これが H を安くする唯一の道。
   *正直な見積り:* 種の初号機コスト = 複数セッション。C の「ゼロ新規動力学」は再現しない
   (C が無料だったのは*それが x2 のグラフだったから*)。
2. **H**(D の後):同じ純 COMB クラス、prefix/状態/head-step が違うだけ → 安価な再具体化。
   **種内での「C 的勝利」の再現。**
