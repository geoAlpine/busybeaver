...
2. **H**:`[保険を実行済み — 結果は予想より強い]` 2026-07-26。A の「D と同じクラス ⇒ 安価」は
   *グラフ同型の意味では* **反証**(状態置換の全探索で同型 0 個 — C を無料にした機構は効かない)。
   しかし **D の rung タイルは H でそのまま発火する**(23040/23040、実軌道 319/319、
   negative control も要求どおり失敗、itinerary は同一の語)。H は D が使い回す状態を2つに割って
   いるだけで、**グラフは違い原子は同じ**。
   ⇒ 機械独立層 `lean/RungCalc.lean` を新設し、タイルを `Atoms T sA sB`(6原子)から証明。
   機械ごとの負担は **6個の閉じた `rfl`** のみ。**`HMachine.rungTile` は `[PROVEN]`**。
   詳細は `D_RUNGTILE_2026-07-26.md` §7。
   **ただし H は `[OPEN]` のまま** — epoch anatomy / entry / milestone 族 / cascade 帰納は未測定。
   島の他候補に `Atoms` を試すのは各 6 `rfl` で済む(= 安価さが *測定可能* になった)。
2b. **`Atoms` 走査 完了**(2026-07-26、`ATOMS_ISLAND_SCAN_2026-07-26.md`):残余 1104 を両向き×
   全6状態で走査し **17/1104 が `Atoms` を完全充足**。17 台すべてのタイルを Lean 化済み
   (`lean/IslandTiles.lean`、自動生成、build 0.6 秒)、17 台すべてで**実軌道上のタイル発火率
   100%**(IN 形配置 231〜398 個/台、300k 歩で停止せず)。`E` は新規ヒット、**`x2` は 1/6** —
   このタイルは x2 の機構ではない。
   *留保:* `Atoms` は 12 遷移エントリのうち 9〜10 を固定するので、17 は「島の広さ」ではなく
   **この1タイルが到達しうる上限**。5/6 の near-miss 1 台(`turn` のみ不足)が最も安い次の一手。
   **17 台すべて `[OPEN]`** — epoch anatomy / entry / milestone 族は未測定。
